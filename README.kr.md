...
`pqai`를 설치하는 방법은 두 가지입니다: 미리 빌드된 바이너리를 다운로드하거나(Go 불필요), 직접 소스에서 빌드하는 방법입니다.

### 방법 A: 미리 빌드된 바이너리 다운로드 (가장 쉬움, Go 불필요)
...
직접 빌드하고 싶거나(또는 아직 릴리스되지 않은 최신 코드를 쓰고 싶다면) 아래 방법 B를 따르세요.
